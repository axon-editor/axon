...
function GhostTab({ path }: { path: string }) {
  ...
  return (
    <div className="flex h-9 shrink-0 select-none items-center gap-1.5 border border-[var(--axon-panel-border)] bg-[var(--axon-panel-background)] px-3 text-[12px] text-[var(--axon-editor-foreground)] opacity-95 shadow-lg">
      <span>{name}</span>
    </div>
  );
}
          ...
          <React.Fragment key={pane.id}>
            ...
            >
              <PaneInstance
                ...
                nativeControlInset={
                  index === 0 ? nativeControlInset : undefined
                }
              />
            </div>
