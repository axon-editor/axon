...
ipcMain.handle(
  "extensions:reload",
  async (_event, folderPath?: string | null): Promise<ExtensionActionResult> => {
    return {
      ok: true,
...
ipcMain.handle(
  ...
  async (
    ...
    // Electron reports shell.openPath failures as a returned string instead of
    // throwing. Returning a normal action result keeps the renderer from showing
    // a vague IPC failure when Finder cannot open the folder for platform or
    // permission reasons.
    if (openError) {
      return {
        ok: false,
